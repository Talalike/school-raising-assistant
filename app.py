...
with st.form("campaign_form"):
    ...
    project_category = st.selectbox("📂 Project category", ["Spazio e Strutture", "Tecnologia", "Arte e Cultura", "Musica", "Sport", "Eventi", "Ambiente", "Inclusione", "Altro"])
    ...
    user_input_3 = st.text_area("🎁 Would you like to offer rewards?", height=80)
    submitted = st.form_submit_button("🚀 Generate Draft")

if submitted:
    with st.spinner("Generating your campaign draft. Please wait..."):
        output = run_pipeline(school_name, project_category, user_input_1, user_input_2, user_input_3)
    ...
    for name, desc_reward, amount in reward_blocks:
        rewards.append({"name": name.strip(), "desc": desc_reward.strip(), "amount": amount.strip()})
    ...
    for i, r in enumerate(rewards):
        ...
        description = st.text_area(f"✏️ Reward {i+1} - Description", value=r["desc"], height=80)
        amount = st.text_input(f"💶 Reward {i+1} - Amount (€)", value=r["amount"])
        reward_inputs.append({"name": name, "desc": description, "amount": amount})
    ...
    st.download_button("📥 Download as PDF", pdf, file_name="campaign_draft.pdf")
